...
module GeneralTransactions
  module Price extend ActiveSupport::Concern
    def total_debit_idr
      @total_debit_idr ||= general_transaction_lines.debit.sum(&:idr)
    end
    def total_debit_usd
      @total_debit_usd ||= general_transaction_lines.debit.sum(&:usd)
    end

    def total_credit_idr
      @total_credit_idr ||= general_transaction_lines.credit.sum(&:idr)
    end
    def total_credit_usd
      @total_credit_usd ||= general_transaction_lines.credit.sum(&:usd)
    end
